...
export const jobLoadAction =
  (pageNumber, keyword = "", location = "") =>
  async (dispatch) => {
    dispatch({ type: JOB_LOAD_REQUEST });
    try {
      const { data } = await axios.get(
        `/api/jobs/show/?pageNumber=${pageNumber}&keyword=${keyword}&location=${location}`
      );
      dispatch({
...
export const myJobLoadAction =
  (userId = "", pageNumber, keyword = "", location = "") =>
  async (dispatch) => {
    dispatch({ type: JOB_LOAD_REQUEST });
    try {
      const { data } = await axios.get(
        `/api/jobs/get-my-jobs/?userId=${userId}&pageNumber=${pageNumber}&keyword=${keyword}&location=${location}`
      );
      dispatch({
        type: JOB_LOAD_SUCCESS,
        payload: data,
